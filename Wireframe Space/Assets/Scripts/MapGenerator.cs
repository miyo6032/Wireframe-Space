...
public class MapGenerator : MonoBehaviour {
    ...
    public void OpenMap()//loads map
    {
        ...
        else
        {
            ...
            newList.Add(GenerateNewMap(nodePrefab.map, Vector2.zero));
    ...
    public void GenerateNewNodes(MapNode node)//Called when the player defeats the previous node
    {
        ...
        if (vertex == origin)//For the first node, add three in order
        {
            ...
            foreach (Vector2 v in nodePos)
            {
                ...
                Map map = GenerateNewMap(node.map, newPos);
                NewNode(map);
            }
    ...
    Map GenerateNewMap(Map map, Vector2 newPos)//Generates the map and the ships, which are generated by three arena sizes - small, med, and large
    {
        int difficultyAddition = map.difficulty + Random.Range(1, map.difficulty + 1);

        int arenaSize = Random.Range(75, 150);

        int totalDifficultyFactor = difficultyAddition * arenaSize;

        int rewardAddition = Mathf.RoundToInt((totalDifficultyFactor / 80.0f));

        List<ShipSave> enemyShips = new List<ShipSave>();

        if (arenaSize < 110)//Small arena - generate a bunch of small ships
        {
            int shipPoints = (int)(totalDifficultyFactor);
            List<ShipSave> possibleShips = GetPossibleShips((float)shipPoints / 20);
            while(shipPoints > 0)
            {
                ShipSave random = possibleShips[Random.Range(0, possibleShips.Count)];
                enemyShips.Add(random);
                shipPoints -= random.shipPoints;
            }
        }
        else if (arenaSize >= 110 && arenaSize < 140)//Med arena - generate a few large ships and small ships
        {
            int shipPoints = (int)(totalDifficultyFactor);
            //Large ships
            List <ShipSave> possibleLargeShips = GetPossibleShips((float)shipPoints / 10);
            while (enemyShips.Count < 3)
            {
                ShipSave random = possibleLargeShips[Random.Range(0, possibleLargeShips.Count)];
                enemyShips.Add(random);
                shipPoints -= random.shipPoints;
            }
            //Small ships
            List<ShipSave> possibleSmallShips = GetPossibleShips((float)shipPoints / 20);
            while (shipPoints > 0)
            {
                ShipSave random = possibleSmallShips[Random.Range(0, possibleSmallShips.Count)];
                enemyShips.Add(random);
                shipPoints -= random.shipPoints;
            }
        }
        else//Large arena - generate a single mega ships and small ships
        {
            int shipPoints = (int)(totalDifficultyFactor);
            //Mega ship
            List<ShipSave> possibleMegaShips = GetPossibleShips((float)shipPoints / 3);
            ShipSave random = possibleMegaShips[Random.Range(0, possibleMegaShips.Count)];
            enemyShips.Add(random);
            shipPoints -= random.shipPoints;
            //Small ships
            List<ShipSave> possibleSmallShips = GetPossibleShips((float)shipPoints / 15);
            while (shipPoints > 0)
            {
                random = possibleSmallShips[Random.Range(0, possibleSmallShips.Count)];
                enemyShips.Add(random);
                shipPoints -= random.shipPoints;
            }
        }
...
public class Map
{
    ...
    public Map(List<ShipSave> ships, int dif, int size, int x, int y, int rew, bool complete)
    {
